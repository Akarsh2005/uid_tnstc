...
const Home = () => {
  ...
  const handleChange = (e) => {
    ...
    // Filter suggestions for source or destination
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!formData.fromPlace || !formData.toPlace || !formData.onwardDate) {
      alert('Please fill in all required fields (Source, Destination, and Onward Date).');
      return;
    }
    console.log('Form submitted:', formData);
    navigate('/fetch-bus', { state: { formData } }); // Redirect to FetchBus page with formData
    // Add API call to backend here if needed
  };

  const handlePackageTourClick = () => {
    navigate('/second'); // Navigate to Package Tour page
  };
    ...
    <div className="min-h-screen bg-[#e6f0fa] font-sans">
      ...
      <nav className="bg-white shadow-lg py-4 px-6 flex justify-between items-center">
        <div className="flex items-center space-x-4">
          <img src="https://www.tnstc.in/images/tamilnadulogo.png" alt="TNSTC Logo" className="h-12" />
          <span className="text-xl font-bold text-gray-800">Tamil Nadu State Transport Corporation (TNSTC)</span>
        </div>
        <div className="space-x-6">
          <button className="btn-nav">Hire a Bus</button>
          <button className="btn-nav">Services</button>
          <button className="btn-nav">Track Ticket</button>
        </div>
      </nav>
      ...
      <div className="relative flex flex-col items-center justify-center min-h-[calc(100vh-80px)] p-6">
        ...
        <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-4xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-4 gap-6">
              <div>
                <label className="block text-gray-700 font-medium mb-2">Onward</label>
                <input
                  type="date"
                  name="onwardDate"
        ...
        <img
          src="https://www.tnstc.in/images/busimage.png"
          alt="Bus Image"
          className="absolute top-1/2 right-10 transform -translate-y-1/2 w-1/3 opacity-70"
        />
      </div>
      ...
      <div className="bg-blue-600 p-8 text-white">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div className="destination-card">
            <img src="https://via.placeholder.com/150" alt="Chennai" className="w-full h-40 object-cover rounded-t-lg mb-4" />
            <h3 className="text-xl font-semibold">Chennai</h3>
            <p className="text-sm">The capital city of Tamil Nadu, known for its culture...</p>
          </div>
          <div className="destination-card">
            <img src="https://via.placeholder.com/150" alt="Bengaluru" className="w-full h-40 object-cover rounded-t-lg mb-4" />
            <h3 className="text-xl font-semibold">Bengaluru</h3>
            <p className="text-sm">The tech hub of Karnataka with vibrant nightlife...</p>
          </div>
          <div className="destination-card">
            <img src="https://via.placeholder.com/150" alt="Kumbakonam" className="w-full h-40 object-cover rounded-t-lg mb-4" />
            <h3 className="text-xl font-semibold">Kumbakonam</h3>
            <p className="text-sm">A historic city in Thanjavur district...</p>
          </div>
          <div className="destination-card">
            <img src="https://via.placeholder.com/150" alt="Tirupati" className="w-full h-40 object-cover rounded-t-lg mb-4" />
      ...
      <footer className="bg-blue-600 text-white p-6 text-center">
        ...
        <p className="text-xs mt-4">© TNSTC. All Rights Reserved. Ver:220925-2128</p> {/* Updated to 09:28 PM IST, 2025-09-22 */}
      </footer>
    </div>
  );
};
